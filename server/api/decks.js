...
const { Chat, Deck, Slide, User } = require('../db/models');
...
router.post('/:deckId/chats', (req, res, next) => {
  Chat.create(req.body)
    .then(message => Chat.findById(message.id, { include: [{ model: User, attributes: ['email'] }, { model: Deck, attributes: ['userId', 'viewable'] }] }))
