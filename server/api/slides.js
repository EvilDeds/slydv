...
router.delete('/:slideId', (req, res, next) => {
  // this route left unsecure because nested promises?
  Slide.findById(req.params.slideId)
      ...
      Deck.findById(+currSlide.deckId, {
        ...
        .then((deck) => {
          ...
          Bluebird.map(deck.slides, (item) => {
            const newPos = item.positionInDeck - 1;
            // console.log(newPos);
            Slide.update({
    ...
    .then(() => Slide.destroy({ where: { id: req.params.slideId } })
